...
import { boundCustomerIds } from '@/api/configure/configureReplenishmentRule';
...
export default {
  ...
  methods: {
    async init(data, disabled) {
      ...
      this.rightTable = [];
      this.getBindShipperList();
      if (data.length > 0) {
        const resp = await shipperList(
          { shipperIds: data, warehouseCode: this.warehouseCode },
          { page: 1, size: 1000 },
        );
    ...
    async getBindShipperList() {
      const result = await boundCustomerIds({});
      this.bindShipperList = result.map((item) => ({ [item.customerId]: item.ruleCode }));
    },
    handleQueryFrom() {
    ...
    async getLeftTable() {
      ...
      const tableData = resp.records.map((res) => {
        const newRes = res;
        newRes.descriptionCode = this.bindShipperList[newRes.id];
        return newRes;
      });
      ...
      tableData.forEach((item) => {
        if (!ids.includes(item.id) && !item.descriptionCode) {
